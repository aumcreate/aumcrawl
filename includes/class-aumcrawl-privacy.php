...
class AumCrawl_Privacy {
	...
	public function add_policy_content() {
		...
		$content = '<p>' . esc_html__( 'This site records visits from automated crawlers, such as search engine and AI crawlers, so the site owner can see which of them read the site.', 'aumcrawl' ) . '</p>'
			. '<p>' . esc_html__( 'What is stored: the crawler name, the date, the paths it requested, and a visit count. No information about human visitors is recorded.', 'aumcrawl' ) . '</p>'
			. '<p>' . esc_html__( 'IP addresses are not stored. When a crawler is checked against the hostname its operator publishes, the address is used during that single request and then discarded. If the check fails, only a shortened network range is kept - the final part of the address is removed first, so it no longer points to an individual connection.', 'aumcrawl' ) . '</p>'
			. '<p>' . esc_html__( 'Records are deleted automatically after the retention period set by the site owner.', 'aumcrawl' ) . '</p>';

		wp_add_privacy_policy_content( __( 'AumCrawl', 'aumcrawl' ), $content );
	}
}
