...
class AumCrawl_Logger {
	...
	public function record() {
		...
		$slug = AumCrawl_Bots::identify( $ua );

		if ( '' === $slug ) {
			return;
		}

		$verdict = AumCrawl_Verify::check( $slug );

		$this->bump_daily( $slug, $verdict );
		$this->bump_page( $slug );

		if ( 'bad' === $verdict ) {
			AumCrawl_Verify::record_suspect( $slug );
		}
		 ...
		 */
		do_action( 'aumcrawl_crawler_seen', $slug, $verdict );
	}
	...
	protected function bump_page( $slug ) {
		...
		if ( $rows >= AumCrawl_Install::PAGE_ROW_CAP ) {
			return;
		}
