...
class AumCrawl_Verify {
	...
	public static function check( $slug ) {
		...
		// Anything that forwards requests -- a CDN, a load balancer, a
		// reverse proxy -- puts its own address on the connection, so reverse
		// DNS would describe that hop rather than the crawler. Detecting the
		// forwarding headers covers every such setup, where a list of known
		// CDN hostnames would only ever cover the ones somebody remembered.
		...
		$key    = 'aumcrawl_v_' . md5( $slug . '|' . $ip . '|' . wp_salt() );
	...
	/**
	 * Is something forwarding requests to this site?
	 *
	 * @return bool
	 */
	protected static function behind_proxy() {
		$forwarding = array(
	...
	public static function client_ip() {
		 ...
		 */
		$ip = (string) apply_filters( 'aumcrawl_client_ip', $ip );
