...
class AumCrawl_Install {
	 ...
	 */
	const CRON_PRUNE = 'aumcrawl_prune';
	...
	public static function activate() {
		...
		update_option( 'aumcrawl_db_version', self::DB_VERSION );
	}
	...
	public static function table( $which ) {
		...
		return $wpdb->prefix . 'aumcrawl_' . $which;
	}
	...
	public static function create_tables() {
		...
		// here. See AumCrawl_Verify::anonymise_ip().
		dbDelta(
	...
	public static function maybe_create_tables() {
		if ( (int) get_option( 'aumcrawl_db_version', 0 ) === self::DB_VERSION ) {
			return;
		}
...
add_action( AumCrawl_Install::CRON_PRUNE, array( 'AumCrawl_Install', 'prune' ) );
add_action( 'plugins_loaded', array( 'AumCrawl_Install', 'maybe_create_tables' ), 1 );
