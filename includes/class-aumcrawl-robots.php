 ...
 */
class AumCrawl_Robots {

	const BEGIN = '# BEGIN AumCrawl';
	const END   = '# END AumCrawl';

	/**
	 * Option recording which crawlers another plugin already claimed.
	 */
	const DEFERRED = 'aumcrawl_deferred';
	...
	public function blocked_tokens() {
		...
		foreach ( AumCrawl_Bots::all() as $slug => $bot ) {
			if ( ! AumCrawl_Bots::is_switchable( $slug ) ) {
				continue;
			}

			if ( aumcrawl_is_blocked( $slug ) && ! empty( $bot['robots'] ) ) {
				$out[ $slug ] = $bot['robots'];
			}
		}
	...
	public static function live_status( $refresh = false ) {
		$cached = get_transient( 'aumcrawl_robots_status' );
		...
		set_transient( 'aumcrawl_robots_status', $status, 5 * MINUTE_IN_SECONDS );
